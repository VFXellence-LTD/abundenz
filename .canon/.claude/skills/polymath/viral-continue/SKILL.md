# Skill: Viral Continue

```yaml
name: viral-continue
description: Resume a Viral campaign after a reviewer requested changes or rejected the prior draft. Reads the review notes, generates a fresh script-level draft addressing them, records a new pending approval, moves the task back to in-review, then HALTS. No publish, no accounts.
triggers:
  - "/viral-continue <campaignId>"
...
- Same hard scope as `/viral-generate`: script-level draft only, no render/voiceover/visual gen, NEVER publish, NEVER touch accounts, Viral brand isolation. The only difference is this run is informed by the prior review notes.

## Inputs

- Same doctrine set as `/viral-generate` (viral-formula, Zrodinger spec, hook-library, title-formula, viral-safeguards policy).
...
1. Resolve `<campaignId>` → `campaignId` + `taskId` (as in `/viral-generate`).
2. Fetch the prior approval's `reviewNotes`. If none found, fall back to behaving exactly like `/viral-generate`.
...
- `viral-generate` — the first-pass loop this mirrors
