...
Print the `gh` command that WOULD run and ask Boss to confirm. Execute write operations (`issue create`, `project item-add`, `project item-edit`, sub-issue linking) only after explicit approval — or inside a Boss-approved execution phase.
...
Default `GITHUB_TOKEN` in GitHub Actions lacks org-level project write permission. Both `add-to-project.yml` and `auto-status.yml` must authenticate with a **fine-grained PAT** stored as repo secret **`PROJECT_PAT`** with project write permission. Boss-run setup step — Claude cannot create PATs or set secrets.

For local `gh` CLI: run `gh auth refresh -s project` to add the `project` scope to the active token.
...
Each sub-issue is atomic — one independently mergeable unit of work. Create with `gh issue create` using `type/task` (or `type/bug`/`type/chore`/`type/infra`), same `area/` label as parent epic, appropriate `priority/` label.
...
`addSubIssue` GraphQL mutation is feature-flagged. Omitting the header causes silent failure — no error message, link not created. Verified during Phase A scaffold (2026-06-17).
...
> **Gotcha:** `updateProjectV2Field` singleSelectOptions must be inlined in the GraphQL query body — do not pass via `--field`/`-f` as a JSON array; the flag coerces to string and fails type validation.
...
Include `#NNN` in the commit body (not subject line) for a GitHub cross-reference:
...
Add `Closes #NNN` in the PR description body. GitHub auto-closes the issue on merge to default branch:
...
These CLAUDE.md files still reference the old `polymath/vault/*` + `apps/dashboard` layout; need updating to `.canon` + `.mission-control`. Tracked as **GitHub issue #14** ("Update stale CLAUDE.md routing tables", `type/infra`, `area/governance`):
...
Docs-only edits, appropriate for Light-tier subagent. Close #14 when done.
