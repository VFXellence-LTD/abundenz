...
| Git | Yes (addon repos) | No (Obsidian vault) |
...
Code projects (dashboard, agents, automation) governed by `dev/DEV-CLAUDE.md`. Mirrors Canon's CANON structure but configured for:

- **GitHub**: [VFXellence-LTD](https://github.com/VFXellence-LTD) organization
- **Task tracking**: Asana (not Jira — personal project, not Halon)
- **MCP**: `@roychri/mcp-server-asana` (token in env var, never in files)

=== FOR CODE WORK, FOLLOW dev/DEV-CLAUDE.md ===
=== FOR BUSINESS WORK, FOLLOW THIS FILE ===

See [[dev/README]] for full development layer overview.

### Polymath Dashboard (Web App)

**Location:** `D:\dev\polymath-dashboard` (separate repo — NOT inside this vault)
...
=== WHEN BOSS SAYS "UPDATE THE DASHBOARD" → WORK IN `D:\dev\polymath-dashboard` ===

**Workflow:** Canon-style — git worktrees for isolation, conventional commits, branch naming per Canon conventions. Use Canon skills (`/start`, `/pr`, `/make`) when applicable.

**Data model lives in:** `polymath-dashboard/src/types/index.ts` + `src/data/ecosystems.ts`
**Business model lives in:** this vault (`_passive_income/controller/dashboard.md`)

The web dashboard visualizes what the Obsidian vault governs. When ecosystem structure changes in this vault, the dashboard app needs corresponding updates to types, data, and UI.
